...
def retrieval_es():
    ...
    # Initialize variable to track selected segment
    ...
    if enable_tokenize:
        ...
        if custom_words:
            ...
            logging.info("Using temporary tokenizer with custom words")
        else:
            # 使用全局分词器
            tokens = rag_tokenizer.tokenize(question).split()
            logging.info("Using global tokenizer without custom words")
        ...
        if tokens:
            ...
            if meaningful_tokens:
                # 找到最后一个有意义token在原始tokens中的最后出现位置
                last_meaningful_token = meaningful_tokens[-1]
                last_meaningful_index = None
                
                # 从后往前找最后一个有意义token的位置
                for i in range(len(tokens) - 1, -1, -1):
                    if tokens[i] == last_meaningful_token:
                        last_meaningful_index = i
                        break
                
                # 从最后一个有意义token开始到结尾的所有tokens（包含该token + 后面的无意义词汇）
                if last_meaningful_index is not None:
                    selected_segment = "".join(tokens[last_meaningful_index:])
                else:
                    selected_segment = last_meaningful_token
                
                # 用于搜索的问题仍然是最后一个有意义token
                question = last_meaningful_token
            else:
                # 如果没有有意义的token，使用最后一个token
                selected_segment = tokens[-1] if tokens else original_question
                question = tokens[-1] if tokens else original_question
            
            logging.info(f"Tokenization - Original: '{original_question}', Tokens: {tokens}, Meaningful: {meaningful_tokens}, Selected segment: '{selected_segment}', Search question: '{question}'")
        else:
            logging.warning(f"No tokens found for question: '{original_question}'")
            selected_segment = original_question
    ...
    try:
        ...
        if not use_embedding:
            ...
            # Add selected segment if tokenize was enabled
            if enable_tokenize and selected_segment is not None:
                ranks["selected_segment"] = selected_segment
